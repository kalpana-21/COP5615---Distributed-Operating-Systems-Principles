...
start() ->
  ...
  register(main, spawn(project3, startChord, [NoNodes, NoReqs])).
...
pingAndKill(Peers, NumNodes, NumRequest, M, Stat) ->
  register(monitor, spawn(project3, completed, [NumNodes * NumRequest, 0])),
...
joinChord(Peers, TotPeers, M, Stat) ->
  ...
  Pid = spawn(project3, node, [Hash, M, Peers, dict:new()]),
